...
class AmazonCAScraper(BaseScraper):
    def extract_data(self, page: Page) -> Dict:
        """Extract data using Playwright for Amazon.ca"""
        
        # --- Set locale to en-CA ---
        try:
            page.context.set_extra_http_headers({"Accept-Language": "en-CA"})
        except Exception as e:
            print(f"[AmazonCA Scraper] Could not set locale: {e}")
        ...
        price_text = ""
        # Price selectors for amazon.ca (often uses .a-price-whole, .a-price-fraction)
        ...
        try:
            ...
            print(f"[AmazonCA Scraper] Could not extract seller details: {e}")
        ...
        try:
            ...
            print(f"[AmazonCA Scraper] Extracted {len(recent_reviews)} review snippets.")
        except Exception as e:
            print(f"[AmazonCA Scraper] Could not extract reviews: {e}")
    ...
    def extract_data_fallback(self, html: str) -> Dict:
        ...
        try:
            ...
            if not seller_name:
             ...
             print(f"[AmazonCA Fallback] Could not extract seller details: {e}")
