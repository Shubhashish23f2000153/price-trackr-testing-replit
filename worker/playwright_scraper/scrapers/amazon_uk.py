...
class AmazonUKScraper(BaseScraper):
    def extract_data(self, page: Page) -> Dict:
        """Extract data using Playwright for Amazon.co.uk"""
        
        # --- Set locale to en-GB ---
        try:
            page.context.set_extra_http_headers({"Accept-Language": "en-GB"})
        except Exception as e:
            print(f"[AmazonUK Scraper] Could not set locale: {e}")
        ...
        price_text = ""
        # Price selectors for amazon.co.uk (often uses .a-price-whole, .a-price-fraction)
        ...
        if "£" not in price_text and page.locator('.a-price-symbol').first.inner_text().strip() == "£":
             try:
        ...
        try:
            ...
            print(f"[AmazonUK Scraper] Could not extract seller details: {e}")
        ...
        try:
            ...
            print(f"[AmazonUK Scraper] Extracted {len(recent_reviews)} review snippets.")
        except Exception as e:
            print(f"[AmazonUK Scraper] Could not extract reviews: {e}")

        return {
            "title": title,
            "price": self.normalize_price(price_text) if price_text else 0,
            "currency": "GBP", # <-- UK Scraper
    ...
    def extract_data_fallback(self, html: str) -> Dict:
        ...
        try:
            ...
            if not seller_name:
             ...
             print(f"[AmazonUK Fallback] Could not extract seller details: {e}")
