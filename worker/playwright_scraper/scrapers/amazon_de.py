...
class AmazonDEScraper(BaseScraper):
    def extract_data(self, page: Page) -> Dict:
        """Extract data using Playwright for Amazon.de"""
        
        # --- Set locale to de-DE ---
        try:
            page.context.set_extra_http_headers({"Accept-Language": "de-DE,de;q=0.9,en;q=0.8"})
        except Exception as e:
            print(f"[AmazonDE Scraper] Could not set locale: {e}")
        ...
        if "€" not in price_text:
             try:
                ...
                if price_symbol == "€":
                    ...
                    price_text = f"{price_whole},{price_fraction}" # Use comma for DE
             except:
                pass # Use whatever we got in price_text

        availability = "Unknown"
        try:
            # German availability text
            avail_elem = page.locator('#availability span').first
            if avail_elem: 
                availability = avail_elem.inner_text().strip()
            else:
                # Check for "Auf Lager" (In stock)
                stock_elem = page.locator('div[data-csa-c-id="delivery-messaging"] span[data-csa-c-type="element"]').first
                if "Auf Lager" in stock_elem.inner_text():
        ...
        try:
            # German merchant info: "Verkauf durch" (Sold by)
            ...
            if rating_text_elem:
                 ...
                 # German: "% positiv" and "Bewertungen"
                 rating_match = re.search(r'(\d+)% positiv.*?\((\d+,?\d*.?\d*) Bewertungen\)', rating_text)
                 if not rating_match: # Fallback to English
                      rating_match = re.search(r'(\d+)% positive.*?\((\d+,?\d*) ratings\)', rating_text)
                 
                 if rating_match:
                     seller_rating = f"{rating_match.group(1)}% Positive"
                     seller_review_count = rating_match.group(2).replace('.', '').replace(',', '') # Handle 1.000
        except Exception as e:
            print(f"[AmazonDE Scraper] Could not extract seller details: {e}")
        ...
        try:
            ...
            print(f"[AmazonDE Scraper] Extracted {len(recent_reviews)} review snippets.")
        except Exception as e:
            print(f"[AmazonDE Scraper] Could not extract reviews: {e}")
        ...
        return {
            ...
            "in_stock": "auf lager" in availability.lower() or "in stock" in availability.lower(),
            "url": self.url,
            "image_url": image_url,
    ...
    def normalize_price(price_str: str) -> int:
        ...
        price_str = price_str.replace(',', '.') # Replace decimal comma
    ...
    def extract_data_fallback(self, html: str) -> Dict:
        ...
        try:
            ...
            if not seller_name:
             ...
             print(f"[AmazonDE Fallback] Could not extract seller details: {e}")
        ...
        return {
            ...
            "in_stock": "auf lager" in avail_text.lower() or "in stock" in avail_text.lower(),
            "url": self.url,
            "image_url": image_url,
            "seller_name": seller_name,
            "seller_rating": None,
